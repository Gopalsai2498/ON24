...
#Survey
outfile = 'D:/on24/ON24_py/Event_Level/Surveys/'
...
for x in df_eventids:
    ...
    col_csv = outfile+'survey_column_list.csv'
    csv_main_cols = list(pd.read_csv(col_csv)['0'])
    df_main_cols =  pd.DataFrame(columns=csv_main_cols)
    
    r = requests.get('https://api.on24.com/v2/client/35078/event/'+str(x)+'/survey', headers=header, params=header)
    ...
    if i == 1:
        df_final.fillna('null').to_csv(outfile+'survey_event_level_jan_aug_2020.csv', sep=',', encoding='utf-8', mode='a', index=False)

    else:
        df_final.fillna('null').to_csv(outfile+'survey_event_level_jan_aug_2020.csv', sep=',', encoding='utf-8', header=None, mode='a', index=False)

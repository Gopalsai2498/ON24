...
#metadata_&_usage
outfile = 'D:/on24/ON24_py/Event_Level/Metadata_&_Usage/'
...
for x in df_eventids:
    ...
    col_csv = outfile+'metadata_&_usage_column_list.csv'
    ...
    if i == 1:
        df_final.fillna('null').to_csv(outfile+'metadata_&_usage_event_level_jan_aug_2020.csv', sep=',', encoding='utf-8', mode='a', index=False)

    else:
        df_final.fillna('null').to_csv(outfile+'metadata_&_usage_event_level_jan_aug_2020.csv', sep=',', encoding='utf-8', header=None, mode='a', index=False)

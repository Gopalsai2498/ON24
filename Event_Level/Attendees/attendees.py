...
#attendees
outfile = 'D:/on24/ON24_py/Event_Level/Attendees/'
...
for x in df_eventids:
    ...
    r = requests.get('https://api.on24.com/v2/client/35078/event/'+str(x)+'/attendee', headers=header, params=header)
    ...
    if i == 1:
        df_final.fillna('null').to_csv(outfile+'attendee_event_level_jan_aug_2020.csv', sep=',', encoding='utf-8', mode='a', index=False)

    else:
        df_final.fillna('null').to_csv(outfile+'attendee_event_level_jan_aug_2020.csv', sep=',', encoding='utf-8', header=None, mode='a', index=False)

...
#Sub-Clients
outfile = 'D:/on24/ON24_py/By_Account/Sub-clients/'

col_csv = outfile+'sub_client_column_list.csv'
csv_main_cols = list(pd.read_csv(col_csv)['0'])
df_main_cols =  pd.DataFrame(columns=csv_main_cols)

r = requests.get('https://api.on24.com/v2/client/35078', headers=header, params=header)
...
if not df_final.empty:
    df_final.fillna('null').to_csv(outfile+'sub_client_by_account_jan_aug_2020.csv', sep=',',encoding='utf-8',mode='a',index=False)